...
class CaptioningRNN(object):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        vocab_size = len(word_to_idx)  # 这里是词库，不是字库
        ...
        self.params['W_proj'] /= np.sqrt(input_dim)  # input_dim :   Dimension D
        self.params['b_proj'] = np.zeros(hidden_dim)  # (H,)

        # Initialize parameters for the RNN
        dim_mul = {'lstm': 4, 'rnn': 1}[cell_type]
        self.params['Wx'] = np.random.randn(wordvec_dim, dim_mul * hidden_dim)
        self.params['Wx'] /= np.sqrt(wordvec_dim)  # Dimension W  Wx:(D, H)
        self.params['Wh'] = np.random.randn(hidden_dim, dim_mul * hidden_dim)
        self.params['Wh'] /= np.sqrt(hidden_dim)  # Dimension H  Wh:(H,H)
    ...
    def loss(self, features, captions):
        ...
        # word and this is what we will expect the RNN to generate.

        # These are offset by one relative to each other because the
        # RNN should produce word (t+1) after receiving word t.
        #
        ...
        # 一次forward:
        # (1)
        # features: (N,D) input 输出初始化的hidden state (N,H)
        initial_h = np.dot(features, W_proj) + b_proj

        # (2)
        # captions_in 相当于 input (N,T) out: (N,T,D) W_embed:(V,D)
        # 也就是说那么输出embed_word 就变成了RNN的输入 input
        embed_word, embed_word_cache = word_embedding_forward(captions_in, W_embed)

        # (3)
        if self.cell_type == 'rnn':
            # rnn_forward(x, h0, Wx, Wh, b) return h, cache    输出隐层h (N,T,H)
        ...
        # (4)
        # 转换成word_embedding的格式  temporal_affine_forward(x(N,T,D), w, b) return out(N,T,M)  相当于 (N,T,H) ,Whh的shape , cache
    ...
    def sample(self, features, max_length=30):
        """
        test-time 运行一次前向传播，根据输入input 得到输出的caption
        ...
        """
        # N代表传进来的图片数和对应的feature分部，已经完成了特征定位。。
        N = features.shape[0]
        captions = self._null * np.ones((N, max_length), dtype=np.int32)
        # captions.shape  : (2L, 30L)  all 0
        # W_embed.shape :(1004L, 256L)
        # features:(N,D)
        ...
        prev_c = np.zeros(prev_h.shape)  # hidden_state 的shape
        ...
        for i in range(max_length):
            ...
            # x:(2L, 256L) 两个数据    W_embed:(V,D)(1004L, 256L) current_word_index:是list [1,1]
            x = W_embed[current_word_index]  #转化为word_vector
            if self.cell_type == 'rnn':
            ...
            word_vectors, cache = temporal_affine_forward(h_next, W_vocab, b_vocab) #word_vectors(N, T, M):(2L, 1L, 1004L)
            captions[:, i] = list(np.argmax(word_vectors, axis=2)) #(N,D) sample的最终输出i代表第i个词的向量
            current_word_index = captions[:, i] #获取当前词，用来计算下一个词
